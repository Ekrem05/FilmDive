...
export default function Card({ subject, movie, lazyLoad }) {
  return (
    <Link to={`/${subject}/${movie.id}`} className="">
      <motion.li
        className="w-32 md:w-64 flex flex-col hover:bg-primary p-4 transition-colors duration-200  rounded-xl pb-2 hover:cursor-pointer "
        whileHover={{ scale: 0.9 }}
      >
        <LazyImage path={movie.posterPath} lazyLoad={lazyLoad} />
        <p className=" text-primaryText lg:text-lg text-sm  text-center  overflow-ellipsis overflow-hidden whitespace-nowrap pl-3 pr-3">
          {movie.title}
        </p>
